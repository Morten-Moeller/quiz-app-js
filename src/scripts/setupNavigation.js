...
export default function setupNavigation(cards) {
  ...
  const footer = getElement('[data-footer]')
  footer.append(navigation)
  setupNavigationEvent(navigation, cards)
}

function setupNavigationEvent(navigation, array) {
  ...
  function handleNavIcon(navIcon) {
    ...
    icon.addEventListener('click', event => {
      ...
      const main = getElement('[data-main]')
      main.innerHTML = ''
      if (icon.dataset.nav === 'home') {
        renderQuizCard(cards)
      } else if (icon.dataset.nav === 'create') {
        setupNewCardForm(cards)
      }
    })
  }
}
